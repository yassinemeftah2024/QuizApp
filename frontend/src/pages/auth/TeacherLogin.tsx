...
import { getClasses, getMatieres, type ClasseWithStats, type MatiereDTO } from '@/services/adminService'
...
export default function TeacherLogin({ onLogin, onRegister, onBack, onSwitchRole, authError }: Props) {
  ...
  useEffect(() => {
    Promise.all([getMatieres(), getClasses()])
      .then(([subjectData, classData]) => {
        setMatieres(subjectData)
        setClasses(classData)
      })
      .catch(() => setError('Impossible de charger les matières et classes.'))
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (tab === 'login') {
      ...
      if (!name || !email || !password || !etablissement || selectedMatieres.length === 0 || selectedClasses.length === 0) {
        setError('Veuillez remplir tous les champs de l’inscription enseignant.');
        return
      }
    }
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
            {['login', 'register'].map(item => (
              <button key={item} type="button" onClick={() => setTab(item as 'login' | 'register')} style={{
                flex: 1,
                padding: '10px 14px',
                borderRadius: 100,
                border: '1px solid rgba(255,255,255,0.12)',
                background: tab === item ? 'rgba(0,229,255,0.16)' : 'transparent',
                color: tab === item ? '#fff' : 'rgba(240,230,255,0.6)',
                cursor: 'pointer',
                fontFamily: 'Nunito',
                fontWeight: 700,
              }}>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <>
              <div style={{ marginBottom: 18 }}>
                ...
                <input
                  ...
                  placeholder="Leila Ben Ali"
                  style={{ width: '100%', padding: '12px 16px', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                />
              </div>

              <div style={{ marginBottom: 18 }}>
                <label style={{ fontSize: 13, fontWeight: 700, color: 'rgba(240,230,255,0.7)', display: 'block', marginBottom: 7, fontFamily: 'Nunito' }}>School</label>
                <input
                  value={etablissement} onChange={e => setEtablissement(e.target.value)}
                  placeholder="Lycée Pasteur"
                  style={{ width: '100%', padding: '12px 16px', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
                />
              </div>

              <div style={{ marginBottom: 18 }}>
                <label style={{ fontSize: 13, fontWeight: 700, color: 'rgba(240,230,255,0.7)', display: 'block', marginBottom: 7, fontFamily: 'Nunito' }}>Subjects</label>
                <div style={{ display: 'grid', gap: 10 }}>
                  {matieres.map(subject => (
                    <label key={subject.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#090B1A', padding: '10px 14px', borderRadius: 12, cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={selectedMatieres.includes(subject.id ?? 0)}
                        onChange={() => {
                          setSelectedMatieres(prev => prev.includes(subject.id ?? 0)
                            ? prev.filter(id => id !== subject.id)
                            : [...prev, subject.id ?? 0])
                        }}
                      />
                      <span style={{ color: '#F0E6FF', fontSize: 14 }}>{subject.nom}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 18 }}>
                <label style={{ fontSize: 13, fontWeight: 700, color: 'rgba(240,230,255,0.7)', display: 'block', marginBottom: 7, fontFamily: 'Nunito' }}>Classes</label>
                <div style={{ display: 'grid', gap: 10 }}>
                  {classes.map(classe => (
                    <label key={classe.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#090B1A', padding: '10px 14px', borderRadius: 12, cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={selectedClasses.includes(classe.id)}
                        onChange={() => {
                          setSelectedClasses(prev => prev.includes(classe.id)
                            ? prev.filter(id => id !== classe.id)
                            : [...prev, classe.id])
                        }}
                      />
                      <span style={{ color: '#F0E6FF', fontSize: 14 }}>{classe.nom}</span>
                    </label>
                  ))}
                </div>
              </div>
          ...
          >
            {loading ? '⚡ Connecting…' : tab === 'login' ? '⚡ Enter Dashboard' : '🌟 Register'}
          </button>
        </form>
